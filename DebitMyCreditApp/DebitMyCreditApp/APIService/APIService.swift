...
struct APIService {
    ...
    static func register(email: String, password: String, completion: @escaping (Result<[String: Any], Error>) -> Void) {
    ...
    static func logout(completion: @escaping () -> Void) {
        // Clear user session/token if locally stored
        // If server-side session handling is added, call the logout endpoint here
        completion()
    }
